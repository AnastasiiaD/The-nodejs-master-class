...
workers.validateCheckData = function (originalCheckData) {
    ...
    originalCheckData.method = typeof(originalCheckData.method) == 'string' && ['get', 'put', 'pot', 'delete'].indexOf(originalCheckData.method) >-1 ? originalCheckData.method : false;
...
workers.alertUserToStatusChange = function (newCheckData) {
    var msg = 'Alert: Your check for' + newCheckData.method.toUpperCase() + '' + newCheckData.url + 'is currently' + newCheckData.state;
    helpers.sendTwilioSms(newCheckData.userPhone, msg, function (err) {
        if(!err) {
            debug("Success was alerted to a status change in their check via sms", msg);
        } else {
...
workers.rotateLogs = function () {
    ...
    _logs.list(false, function (err, logs) {
        if(!err && logs && logs.length)  {
            ...
            debug({'Error': 'Could not fined any logs to rotate'})
        }
    });
};
...
workers.init = function () {
    ...
    // Compress all mthe logs immediately
    workers.rotateLogs();
    
    // Call the compression loop so logs will be compressed on
